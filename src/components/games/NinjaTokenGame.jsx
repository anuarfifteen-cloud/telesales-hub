...
const WALL_PAD = 28;
const NINJA_FONT = 88;         // 2x the original size so the ninja icon reads clearly
const ENEMY_FONT = 80;         // 2x the original size so oni / bomb read clearly
...
// Difficulty: relaxed playable base, gentle gradual increase +step every 500 pts
...
export default function NinjaTokenGame({ user /* , onUserUpdate */ }) {
  ...
  const update = useCallback(
    (dt) => {
      ...
      const spawnInterval = Math.max(1.0, 1.6 - tier * 0.05);
      w._speed = speed;
      ...
      for (const e of w.entities) {
        e.y += (e.type === "bomb" ? speed * 1.08 : speed) * dt;
      }
