...
export default function AnnouncementPanel({ announcements, user, onClose }) {
  
  const active = announcements
    .filter(isActive)
    // ─── ADDED: Filter out announcements meant for other users ───
    .filter((a) => {
      if (!a.targetUserId) return true; // Global announcement
      return a.targetUserId === user?.id; // Only show if it matches this user
    })
