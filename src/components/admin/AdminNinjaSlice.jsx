...
export default function AdminNinjaSlice() {
  ...
  const { data: scores = [], isLoading } = useQuery({
    queryKey: ["ninjaSliceScoresAdmin"],
    queryFn: () => base44.entities.NinjaTokenScore.list("-score", 50),
  });

  const { data: allUsers = [], refetch: refetchUsers } = useQuery({
    queryKey: ["allUsersAdminNinja"],
    queryFn: () => base44.entities.User.list(),
  });

  const { data: appSettingsRows = [] } = useQuery({
    queryKey: ["appSettingsAdminNinja"],
    queryFn: () => base44.entities.AppSettings.list(),
  });

  const appSettings = appSettingsRows[0] || {};
  const hideLeaderboard = !!appSettings.hide_ninja_slice_leaderboard;
    ...
    try {
      ...
      if (appSettingsRows[0]) {
        await base44.entities.AppSettings.update(appSettingsRows[0].id, {
          hide_ninja_slice_leaderboard: next,
        });
      } else {
      ...
      queryClient.invalidateQueries({ queryKey: ["appSettingsAdminNinja"] });
      toast.success(
        next
          ? "🔒 Ninja Slice leaderboard hidden (Mystery Mode)"
          : "👁️ Ninja Slice leaderboard is now visible"
      );
    } catch (err) {
  ...
  const champUserIds = new Set(allUsers.filter((u) => u.is_defending_champ_ninja).map((u) => u.id));
  const champs = allUsers.filter((u) => u.is_defending_champ_ninja);
  const nonChamps = allUsers.filter((u) => !u.is_defending_champ_ninja);
  ...
  const syncChampSettings = async (championUserIds) => {
    ...
    if (settings) {
      await base44.entities.AppSettings.update(settings.id, { defending_champ_ninja_ids: championUserIds });
    } else {
      await base44.entities.AppSettings.create({ defending_champ_ninja_ids: championUserIds });
    }
    queryClient.invalidateQueries({ queryKey: ["appSettingsAdminNinja"] });
  };
  ...
  const toggleChamp = async (u, makeChamp) => {
    ...
    try {
      await base44.entities.User.update(u.id, { is_defending_champ_ninja: makeChamp });
  ...
  const handleEndSeason = async () => {
    ...
    try {
      ...
      const eligible = scores.filter((s) => !userMap[s.user_id]?.is_defending_champ_ninja);
      ...
      for (let i = 0; i < top3.length; i++) {
        ...
        await base44.entities.TokenTransaction.create({
          ...
          amount: tokenReward,
          source: `Ninja Slice Season Reward — #${i + 1} Place (${entry.score} pts)`,
          timestamp: new Date().toISOString(),
        });
      ...
      for (const u of freshUsers) {
        if (u.is_defending_champ_ninja) {
          await base44.entities.User.update(u.id, { is_defending_champ_ninja: false });
        }
      }

      // Set new defending champ
      if (newChampUserId) {
        await base44.entities.User.update(newChampUserId, { is_defending_champ_ninja: true });
      }
      ...
      queryClient.invalidateQueries({ queryKey: ["ninjaSliceScoresAdmin"] });
      queryClient.invalidateQueries({ queryKey: ["allUsersAdminNinja"] });
      toast.success("✅ Ninja Slice season ended! Top 3 eligible players paid & leaderboard wiped.");
    } catch (err) {
      toast.error("Error: " + err.message);
    } finally {
      setProcessing(false);
  ...
  const seasonLabel = new Date().toLocaleString("default", { month: "long", year: "numeric" });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-amber-500 to-yellow-600 rounded-2xl p-4 text-white">
        ...
        <p className="font-black text-lg">🥷 Ninja Slice</p>
        <p className="text-xs opacity-80">Season — {seasonLabel} · Manage the seasonal Ninja Slice leaderboard</p>
      </div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-bold text-sm text-foreground">👁️ Show Ninja Slice Leaderboard to Players</h3>
              <span
            ...
            <p className="text-xs text-muted-foreground mt-1.5">
              When hidden, the game stays fully playable — players can keep slicing and still see their own personal best, but all other competitors' ranks and scores stay secret until the season deadline.
            </p>
          </div>
          <button
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-2.5 ${
                    isChamp ? "opacity-60 bg-muted/30" : i < 3 ? "bg-amber-50/50 dark:bg-amber-950/10" : ""
                  }`}
                >
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-sm font-black text-amber-600 dark:text-amber-400 tabular-nums">{s.score} pts</p>
      ...
      <AlertDialog open={showConfirm} onOpenChange={setShowConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>🚨 End Ninja Slice Season & Payout?</AlertDialogTitle>
            <AlertDialogDescription>
